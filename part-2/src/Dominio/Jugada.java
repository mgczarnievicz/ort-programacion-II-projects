...
public class Jugada implements Cloneable, Serializable {
    //Variables
    ...
    public String logicaValida(int[] pos, char turno, char noTurno) {
        ...
        //Valida que en el casillero de salida este una pieza del turno correspondiente.
        if (!this.getTablero().getTablero()[pos[0]][pos[1]].colorPieza(turno)) {
            sePuede = false;
            tipoFalla = "NO ESTA LA PIEZA INDICADA";
        } else {

            //Me fijo en que tipo de diagonal se encuentra la jugada indicada.
            ...
            //Dependiendo del tipo de la diagonal recorro para ver si esta vacia entre los dos casilleros
            if (sePuede) {                
                //Tipo de diagonal: /
                if (tipoDeDiagonal == -1) {
                    for (int i = 1; i < (Math.abs(pos[2] - pos[0])); i++) {
                        if (!(this.getTablero().getTablero()[Math.max(pos[0], pos[2]) - i][Math.min(pos[1], pos[3]) + i].colorPieza(' '))) {
                            sePuede = false;
                            tipoFalla = "EL CAMINO NO ES VACÍO";
                        }
                    }
                    //Tipo de diagonal: \
                } else {
            ...
            /*Si la diagonal entre los dos casilleros es vacio.
            Valido: que en el casillero de llegada este la pieza del turno contrario
                    que el casillero de llegada sea vacio, haciendo entonces las validaciones correspondientes*/
            if (sePuede) {

                //Valido que en la posición de llegada haya una pieza del turno contrario
                if (this.getTablero().getTablero()[pos[2]][pos[3]].colorPieza(turno)) {
                    tipoFalla = "LA CASILLA YA ESTA OCUPADA";
                }

                // Si en la posición de llegada hay un espacio, primero me fijo si no podia comer con esa ficha y luego me fijo si donde llego amenazo.
                if (this.getTablero().getTablero()[pos[2]][pos[3]].colorPieza(' ')) {

                    //El metodo  recorrerDiagonal me muestra que fichas hay en los extremos de las diagonales
                    //Recorro la diagonal para ver si ese alfil podia comer
                    ...
                    if (comp[0].colorPieza(noTurno) || comp[1].colorPieza(noTurno) || compB[0].colorPieza(noTurno) || compB[1].colorPieza(noTurno)) {
                        ...
                        //Valido que la pieza en la posición de llegada puede comer
    ...
    Validan que ambos sean una letra y un mumero.
    ...
    public String entradaVali() {
        ...
        //Si ambas cosas (el código es de 2 caracteres y estos existan en el tablero, entonces el código entrante es válido.
    ...
    //Valido que existan las posiciones
    public String existenPos() {
        String piezas[] = this.getJugada().split(" ");
        boolean bien = false;
        String tipoFalla = "BIEN";

        //Separo la letra y el número de la jugada
    ...
    /* Método que se le pasa un String Validado (ya se verifico que es correcto),
     y me devuelve las posiciones x, y , x1, y1
    ...
    public String colocoVali() {
        ...
        if (entradaCorrecta) {
            if (validarPos(piezas[0].charAt(0), (int) piezas[0].charAt(1)) && (piezas[1].charAt(0) == 'B' || piezas[1].charAt(0) == 'N')) {
                ...
                //Escribe el error
                tipoFalla = "No existe la posición o no ingreso una pieza Blanca o Negra";
            }
        }
        return tipoFalla;
    }
